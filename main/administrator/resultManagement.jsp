...
<head>
    ...
    <link rel="stylesheet" href="<%=request.getContextPath()%>/static/css/Deng/resultManagement.css">
...
<div class="containers">
    ...
    <div class="youxia">
        <div class="task_container left-horizon right-horizon"><!--编写添加任务这里之后需要进行修改，变成遍历输入-->
            ...
            %>
            <%
                for (int i=12*(currentPage-1);i<Math.min(task_list.size(),10*(currentPage-1)+10);i++) {
                    Task a=task_list.get(i);
            %>

                <div class="container1" onclick="window.location.href= '<%=request.getContextPath()%>/main/administrator/taskDetails.jsp?taskId=<%= a.getTaskId() %>'">
        ...
        <div class="pageConfig">
            <a style="margin-left: 32%" href="<%=request.getContextPath()%>/main/administrator/resultManagement.jsp?page=1">首页</a>
            <a href="<%=request.getContextPath()%>/main/administrator/resultManagement.jsp?page=<%=(currentPage > 1) ? (currentPage - 1) : 1 %>">上一页</a>
            <a href="<%=request.getContextPath()%>/main/administrator/resultManagement.jsp?page=<%=currentPage%>">当前:<%=currentPage%>/<%=task_list.size()/10+1%></a>
            <a href="<%=request.getContextPath()%>/main/administrator/resultManagement.jsp?page=<%= Math.min(currentPage + 1,task_list.size()/10+1) %>">下一页</a>
            <a href="<%=request.getContextPath()%>/main/administrator/resultManagement.jsp?page=<%=task_list.size()/10+1%>">尾页</a>
        </div>
